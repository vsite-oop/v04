...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(concrete_insect)
		{
			...
			insect& i = c;
			animal& a = c;
			Assert::AreEqual("cockroach"s, a.species());
		...
		TEST_METHOD(concrete_bird)
		{
			...
			bird& b = s;
			animal& a = s;
			Assert::AreEqual("sparrow"s, a.species());
		...
		TEST_METHOD(concrete_spider)
		{
			...
			spider& s = t;
			animal& a = t;
			Assert::AreEqual("tarantula"s, a.species());
		...
		{
			leg_counter lc;
			Assert::AreEqual("cockroach"s, lc.add_animal(animal_factory(1).get()));
			Assert::AreEqual("sparrow"s, lc.add_animal(animal_factory(2).get()));
			Assert::AreEqual("tarantula"s, lc.add_animal(animal_factory(3).get()));
